...
- **`expo prebuild` fails on Node 22 under Linux** with `Could not find MIME for Buffer <null>`. Node 20 and 24 both work, and macOS is unaffected. It comes from `@expo/image-utils`, which pins `jimp-compact@0.16.1` in its newest published version, so there is nothing to upgrade to. CI is pinned to Node 24 — never move it to 22.
